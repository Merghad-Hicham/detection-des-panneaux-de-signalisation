...
import org.javaopencvbook.utils.CheckCrossLine;
...
public class CountPan {
    private Mat image;
    public List<MatOfPoint> CarsContours = new ArrayList<MatOfPoint>();
    private int areaThreshold;
    private int CarSizeThreshold;
    ...
    MatOfPoint contourCar;

    public CountPan(int areaThreshold, int CarSizeThreshold, Point lineCount1, Point lineCount2, Point lineSpeed1, Point lineSpeed2, boolean crossingLine, boolean crossingSpeedLine) {
        this.areaThreshold = areaThreshold;
        this.CarSizeThreshold = CarSizeThreshold;
    ...
    public Mat findAndDrawContours(Mat image, Mat binary) {
        ...
        for (int i = 0; i < contours.size(); i++) {
            ...
            if (currentArea > areaThreshold) {
                CarsContours.add(contours.get(i));
    ...
    public boolean isCarToAdd() {
        for (int i = 0; i < CarsContours.size(); i++) {
            Rect rectangle = Imgproc.boundingRect(CarsContours.get(i));
            if (checkRectLine.rectContainLine(rectangle)) {
                contourCar = getCarsContours().get(i);
                countingFlag = true;
                break;
            }
        }
    ...
    public String classifier() {
        double currentArea = Imgproc.contourArea(contourCar);
        if (currentArea <= (double) CarSizeThreshold)
            return "Car";
        else if (currentArea <= 1.9 * (double) CarSizeThreshold)
            return "Van";
        else return "Lorry";
    }
    ...
    public List<MatOfPoint> getCarsContours() {
        return CarsContours;
    }
